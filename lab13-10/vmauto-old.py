...
#
# NOTES -----------------------------------------------------------
# 1) If you're running VirtualBox on Windows, you'll need win32com,
#    which is included in the Python Extensions for Windows package
# -----------------------------------------------------------------
...
class VBoxAuto(object):
    ...
    def check(self):
        ...
        pinfo('Session state: %s' % self.get_const("SessionState", self.mach.sessionState))
        pinfo('Machine state: %s' % self.get_const("MachineState", self.mach.state))
        return True

    def get_const(self, enum, elem):
        # this lookup fails on Python2.6 - if that happens just return the element number
        try:
            all_vals = self.ctx['const'].all_values(enum)
            for e in all_vals.keys():
                if str(elem) == str(all_vals[e]):
                    return e
        except:
            pass
        return '%d' % elem
    ...
    def winexec(self, user, passwd, args):
        ...
        try:
            try:
                ...
                argstr = ''
            pinfo("Executing '%s' with args '%s'" % (args, argstr))
...
class VMwareAuto(object):
    def __init__(self, vmx):
        ...
        if not os.path.isfile(vmx):
            raise Exception('Cannot find vmx file in ' + vmx)

        for (path, vtype) in vm_paths.items():
        ...
        if self.vmrun is None:
            raise Exception('Cannot find vmrun in ' + ','.join(vm_paths.keys()))
        else:
            print 'Found vmrun (running on %s)' % self.vmtype

    def setuser(self, user, passwd):
        """
        Sets the credentials on the guest machine to use when copying files
        to/from the guest and when executing programs in the guest.
        """
        self.user = user
        self.passwd = passwd
    ...
    def run_cmd(self, cmd, args=[], guest=False):
        """
        Execute a command through vmrun. Additional parameters for commands
        can be set with args[].
        """
    ...
    def revert(self, snapname):
        """
        Revert the virtual machine specified by self.vmx to the given snapshot.
        """
        return self.run_cmd('revertToSnapshot', [snapname])

    def suspend(self):
        """
        Suspend the virtual machine specified by self.vmx. This is usually done
        after executing malware in order freeze the machine's state and obtain
        its physical memory sample.
        """
        return self.run_cmd('suspend')

    def scrshot(self, outfile):
        """
        Take a screen shot of the guest's desktop and save it to the file specified by outfile.
        """
        return self.run_cmd('captureScreen', [outfile], guest=True)

    def copytovm(self, src, dst):
        """
        Copy the src file (src is a path on the host) to dst (dst is a path on the guest).
        """
        if not os.path.isfile(src):
            perror('Cannot locate source file ' + src)
            return
        return self.run_cmd('copyFileFromHostToGuest', [src, dst], guest=True)

    def copytohost(self, src, dst):
        """
        Copy the src file (src is a path on the guest) to dst (dst is a path on the host).
        """
        return self.run_cmd('copyFileFromGuestToHost', [src, dst], guest=True)

    def winexec(self, file, args=''):
        """
        Execute a command in the guest with supplied arguments.
        You can use this to execute malware or existing programs on the guest machine.
        """
        return self.run_cmd(
            'runProgramInGuest',
            [
    ...
    def findmem(self):
        """
        Find the file on the host machine's file system that represents the guest's
        physical memory. This is usually only available when the guest is suspended.
        """
        path = os.path.dirname(self.vmx)
        mems = glob.glob('%s/*.vmem' % (path))
        mems = [m for m in mems if "Snapshot" not in m]
        return mems if len(mems) else ''
